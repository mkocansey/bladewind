...
@endphp
<label id="@if($add_id_prefix)bw-@endif{{$id}}" class="relative text-[10px] uppercase px-[12px] leading-8 tracking-widest whitespace-nowrap inline-block {{$rounded_class}} {{$clearing_css}} bg-{{$color}}-{{$color_weight[$shade]}} text-{{$color}}-{{$text_color_weight[$shade]}} {{$class}}">
{{ $label }}
@if($can_close)
