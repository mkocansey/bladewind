...
@php
    ...
    if(!empty($selected)) {
        $selected_time_array = explode(':', str_replace(' ', '', $selected));
...
@endphp
@if($style == 'icon')
    <div style="width: 120px" class="flex bw-time-icon-container-{{$name}}">
        <x-bladewind::input
                class="bw-time-{{$name}}"
                :name="$name"
                suffix="clock"
        ...
        <div class="flex justify-center pt-4 pb-3">
            ...
            @if($format  == '12')
                <div class="pl-3 space-y-1">
                    <div class="rounded-t-lg font-semibold cursor-pointer bg-gray-100 text-2xl px-4 py-2 hover:bg-gray-300 bw-time-format-am"
                         onclick="toggleFormat('AM', '{{$name}}');setTime_{{$name}}('AM')">
                        {{ __('bladewind::timepicker.AM') }}
                    </div>
                    <div class="rounded-b-lg font-semibold cursor-pointer bg-gray-100 text-2xl px-4 py-2 hover:bg-gray-300 bw-time-format-pm"
...
@else
    ...
    <input type="hidden" class="bw-time-{{$name}}" name="{{$name}}" value="{{str_replace(' ', '', $selected)}}"/>
@endif
