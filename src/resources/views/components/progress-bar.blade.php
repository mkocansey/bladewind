...
@php 
    if ($color == 'gray' && $shade == 'faint') {
        $css_override = '!bg-slate-300';
    }
@endphp
...
<div class="bw-progress-bar {{$css}}">
    ...
    <div class="text-sm {{str_replace('bottom ','text-', $percentage_label_position)}}">
        {{$percentage_prefix}} {{ $percentage}}% {{$percentage_suffix}}
    </div>
    @endif
</div>