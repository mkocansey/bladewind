...
<label class="relative @if($justified=='false')inline-flex @else flex justify-between @endif items-center group" onclick="{!!$onclick!!}">
    ...
    <input type="checkbox" name="{{$name}}" @if($checked=='true')checked@endif @if($disabled=='true')disabled@endif class="absolute left-1/2 -translate-x-1/2 w-full h-full peer appearance-none rounded-md cursor-pointer" />
