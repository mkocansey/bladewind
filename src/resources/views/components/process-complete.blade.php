...
<div class="{{$name}} text-center mt-6 hidden">
    ...
    @endif
    <div class="my-3 text-sm @if($processCompletedAs === 'passed') text-green-600 @else text-red-600 @endif">
        {{ $message }} <span class="error-message"></span>
        <div class="mt-8"></div>
        <x-button type="tiny" onclick="{{$buttonAction}}">{{ $buttonLabel }}</x-button>
    </div>
</div>